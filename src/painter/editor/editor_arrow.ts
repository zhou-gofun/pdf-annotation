...
export class EditorArrow extends Editor {
    ...
    constructor(EditorOptions: IEditorOptions) {
        super({ ...EditorOptions, editorType: Annotation.ARROW })
        this.arrow = null
    ...
    protected mouseDownHandler(e: KonvaEventObject<MouseEvent | TouchEvent>) {
        ...
        this.startPoint = pos ? { x: pos.x, y: pos.y } : { x: 0, y: 0 }
        
        this.arrow = new Konva.Arrow({
            points: pos ? [pos.x, pos.y, pos.x, pos.y] : [0, 0, 0, 0],
            stroke: this.currentAnnotation?.style?.color ?? '#000000',
            strokeWidth: this.currentAnnotation?.style?.strokeWidth ?? 1,
    ...
    protected mouseMoveHandler(e: KonvaEventObject<MouseEvent | TouchEvent>) {
        ...
        const pos = this.konvaStage.getRelativePointerPosition()
        const points = pos ? [this.startPoint.x, this.startPoint.y, pos.x, pos.y] : [this.startPoint.x, this.startPoint.y, this.startPoint.x, this.startPoint.y]
    ...
    protected mouseUpHandler() {
        ...
        if (this.isTooShort()) {
            this.arrow?.destroy()
            group?.id() && this.delShapeGroup(group.id())
            this.arrow = null
            return
        }
        ...
        this.setShapeGroupDone({
            id: group?.id() ?? '',
            color: this.currentAnnotation?.style?.color ?? '#000000',
            contentsObj: {
                text: ''
            }
        })
