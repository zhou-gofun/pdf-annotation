...
export class EditorRectangle extends Editor {
    ...
    constructor(EditorOptions: IEditorOptions) {
        super({ ...EditorOptions, editorType: Annotation.RECTANGLE }) // 调用父类的构造函数
        this.rect = null
        this.vertex = { x: 0, y: 0 }
    }
    ...
    protected mouseDownHandler(e: KonvaEventObject<MouseEvent | TouchEvent>) {
        ...
        this.vertex = pos ? { x: pos.x, y: pos.y } : { x: 0, y: 0 } // 记录鼠标按下时的坐标作为矩形的起始点
    ...
    protected mouseUpHandler() {
        ...
        if (this.isTooSmall()) {
            ...
            group && this.delShapeGroup(group.id()) // 删除矩形所在的组
            this.rect = null
            return
        }
    ...
    private isTooSmall(): boolean {
        const { width, height } = this.rect?.size() ?? { width: 0, height: 0 } // 获取矩形的宽度和高度
