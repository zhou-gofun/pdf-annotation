...
export class EditorCircle extends Editor {
    ...
    constructor(EditorOptions: IEditorOptions) {
        super({ ...EditorOptions, editorType: Annotation.CIRCLE })
        this.ellipse = null
        this.vertex = { x: 0, y: 0 }
    }
    ...
    protected mouseDownHandler(e: KonvaEventObject<MouseEvent | TouchEvent>) {
        ...
        // 确保 pos 不为空，并存储椭圆的起点坐标
        this.vertex = pos ? { x: pos.x, y: pos.y } : { x: 0, y: 0 }
        ...
        this.ellipse = new Konva.Ellipse({
            ...
            stroke: this.currentAnnotation?.style?.color ?? '#000000', // 设置椭圆边框颜色
            strokeWidth: this.currentAnnotation?.style?.strokeWidth ?? 1, // 设置椭圆边框宽度
            opacity: this.currentAnnotation?.style?.opacity ?? 1 // 设置椭圆透明度
        })
    ...
    protected mouseUpHandler() {
        ...
        if (this.isTooSmall()) {
            ...
            group && this.delShapeGroup(group.id())
            this.ellipse = null
            return
        }
    ...
    private isTooSmall(): boolean {
        const { width, height } = this.ellipse?.size() ?? { width: 0, height: 0 }
