...
export class Painter {
  ...
  public readonly onWebSelectionSelected: (
    range: Range | null,
    isOnExistingAnnotation?: boolean
  ) => void;
  public readonly onStoreAdd: (
    annotationStore: IAnnotationStore,
    isOriginal: boolean,
    currentAnnotation: IAnnotationType
  ) => void;
  public readonly onStoreDelete: (id: string) => void;
  public readonly onAnnotationSelected: (
    annotationStore: IAnnotationStore,
    isClick: boolean,
    selectorRect: IRect
  ) => void;
  public readonly onAnnotationChange: (
    annotationStore: IAnnotationStore
  ) => void;
  public readonly onAnnotationChanging: () => void; // 批注正在更改的回调函数
  public readonly onAnnotationChanged: (
    annotationStore: IAnnotationStore,
    selectorRect: IRect
  ) => void; // 批注已更改的回调函数
      ...
      onHighlight: (selection) => {
        Object.keys(selection).forEach((key) => {
          ...
          if (canvas) {
            ...
            let storeEditor = this.findEditor(
              pageNumber,
            ...
            if (!storeEditor) {
              ...
              //   +++++++++++++
              const annotation = this.store.annotation(storeEditor.id);
              if (annotation) {
                this.onAnnotationSelected(annotation, true, { x: 0, y: 0, width: 0, height: 0 });
              }
            }
            if (elements) {
  ...
  private globalKeyUpHandler = (e: KeyboardEvent): void => {
    if (
      e.code === "Escape" &&
      (this.currentAnnotation?.type === Annotation.SIGNATURE ||
        this.currentAnnotation?.type === Annotation.STAMP)
    ) {
  ...
  public initCanvas({
    ...
    try {
      ...
      if (!pageView || !pageView.div || !pageView.viewport) {
        console.warn(
          `Page ${pageNumber} is not ready for canvas initialization`
        );
        return;
      }
  ...
  public activate(
    ...
    console.log(
      `Painting mode active type: ${annotation.type} | pdfjs annotationStorage type: ${annotation.pdfjsEditorType}`
    );
   ...
   */
  public async initAnnotations(
    annotations: IAnnotationStore[],
    loadPdfAnnotation: boolean
  ) {
    // 加载 pdf 文件批注
  ...
  public async highlight(annotation: IAnnotationStore) {
    // 跳转至对应页面位置
    const pageView =
      this.pdfViewerApplication.pdfViewer._pages[annotation.pageNumber - 1] ||
      this.pdfViewerApplication.pdfViewer.getPageView(annotation.pageNumber);
    ...
    const attemptHighlight = (retries: number): void => {
      ...
      if (storeEditor) {
        ...
        if (
          ...
          this.currentAnnotation.type === Annotation.SELECT
        ) {
   ...
   * @param style
   */
  public updateCurrentAnnotationStyle(style: {
    color?: string;
    opacity?: number;
  }) {
  ...
  public disable() {
    this.konvaMap.forEach((konvaStage, pageNumber) => {
      const wrapper = document.querySelector(
        `#${PAINTER_WRAPPER_PREFIX}_page_${pageNumber}`
      ) as HTMLDivElement;
      if (wrapper) {
