import type { Annotation, PDFViewerApplication } from 'pdfjs'
import type { IAnnotationStore } from '../../const/definitions'
import { PdfjsAnnotation } from '../../const/definitions'
...
export class Transform {
    ...
    private async getAnnotations(): Promise<Annotation[]> {
        ...
        const annotationsPromises = Array.from({ length: numPages }, (_, i) =>
            pdfDocument.getPage(i + 1).then((page: { getAnnotations: () => Promise<Annotation[]> }) => {
    ...
    private decodeAnnotation(annotation: Annotation, allAnnotations: Annotation[]): IAnnotationStore | null {
        const decoderMap: Record<string | number, new (options: any) => Decoder> = {
            [PdfjsAnnotation.CIRCLE]: CircleDecoder,
            [PdfjsAnnotation.FREETEXT]: FreeTextDecoder,
            [PdfjsAnnotation.HIGHLIGHT]: HighlightDecoder,
            [PdfjsAnnotation.UNDERLINE]: HighlightDecoder,
            [PdfjsAnnotation.STRIKEOUT]: HighlightDecoder,
            [PdfjsAnnotation.SQUARE]: SquareDecoder,
            [PdfjsAnnotation.INK]: InkDecoder,
            [PdfjsAnnotation.LINE]: LineDecoder,
            [PdfjsAnnotation.POLYGON]: PolygonDecoder,
            [PdfjsAnnotation.POLYLINE]: PolylineDecoder,
            [PdfjsAnnotation.TEXT]: TextDecoder
        }
