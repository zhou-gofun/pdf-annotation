import type { Annotation, PDFViewerApplication, QuadPoint } from 'pdfjs'
...
export abstract class Decoder {
    ...
    protected getComments(annotation: Annotation, allAnnotations: Annotation[]) : IAnnotationComment[]{
        ...
        allAnnotations.forEach((_item) => {
            if(_item.annotationType === PdfjsAnnotation.TEXT && _item.inReplyTo  === annotation.id) {
                reply.push({
                    id: _item.id,
    ...
    public abstract decodePdfAnnotation(
        annotation: Annotation,
        allAnnotations: Annotation[]
      ): IAnnotationStore | null   // ✅ 改成允许 null
}
